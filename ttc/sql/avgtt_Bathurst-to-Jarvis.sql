-- EB KING
select period_name, round(avg(bl)/60,1) bl_min, round(avg(bl),1) bl_sec
from 
(
	-- avg tt per time_bin for corridor
	select period_name, time_bin, sum(sum(avgtt)) over (partition by time_bin) as bl
	from
	(
