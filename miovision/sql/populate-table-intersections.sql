...
SELECT 	TRIM(substring(replace(study_name,'Adelaide/','Adelaide /') FROM '(?:(?!Oct|Nov).)*')) AS intersection_name,
	lat,
	lng
FROM 	miovision.raw_data
GROUP BY lat, lng, TRIM(substring(replace(study_name,'Adelaide/','Adelaide /') FROM '(?:(?!Oct|Nov).)*')), substring(study_name FROM '([a-zA-Z]*)')
